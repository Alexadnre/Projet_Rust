...
fn main() {
    App::new()
        ...
        .add_systems(Update, (slider_system, update_hexagons_and_buildings))
        .run();
}
...
fn setup(mut commands: Commands, asset_server: Res<AssetServer>) {
    ...
    // Slider pour la taille des bâtiments
    commands.spawn((
        TextBundle {
...
fn update_hexagons_and_buildings(
    ...
    // Supprimer les anciens hexagones et bâtiments
    ...
    spawn_buildings(&mut commands, &mut meshes, &mut materials, hex_size, tile_spacing.factor, offset);
}
...
fn spawn_hexagon(
    ...
    let hex_vertices: Vec<Vec2> = (0..6).map(|i| hex_corner(center, size, i)).collect();
...
fn spawn_buildings(
    ...
    let mut occupied = vec![vec![false; GRID_HEIGHT as usize]; GRID_WIDTH as usize];
    let building_types = [
        vec![(0, 0)], // 1 hexagone (carré)
        vec![(0, 0), (1, 0)], // 2 hexagones horizontaux
        vec![(0, 0), (0, 1)], // 2 hexagones verticaux
        vec![(0, 0), (1, 0), (1, 1)], // Forme en L
        vec![(0, 0), (1, 0), (0, 1), (1, 1)], // 4 hexagones (carré 2x2)
    ];

    for q in 0..GRID_WIDTH {
        for r in 0..GRID_HEIGHT {
            if !occupied[q as usize][r as usize] && rand::random::<f32>() < 0.3 { // 30% de chance de spawn
                let building_type = &building_types[rand::random::<usize>() % building_types.len()];
                let mut can_place = true;

                // Vérifier si le bâtiment peut être placé
                for &(dq, dr) in building_type {
                    let new_q = q + dq;
                    let new_r = r + dr;
                    if new_q >= GRID_WIDTH || new_r >= GRID_HEIGHT || occupied[new_q as usize][new_r as usize] {
                        can_place = false;
                        break;
                    }
                }
                ...
                if can_place {
                    // Marquer les hexagones comme occupés
                    for &(dq, dr) in building_type {
                        occupied[(q + dq) as usize][(r + dr) as usize] = true;
                    }

                    // Générer le bâtiment
                    spawn_building(commands, meshes, materials, hex_size, spacing_factor, offset, q, r, building_type);
                }
            }
        }
    }
...
fn spawn_building(
    ...
    for &(dq, dr) in shape {
        let center = hex_position(q + dq, r + dr, hex_size, spacing_factor) + offset;
        let vertices: Vec<Vec2> = (0..6).map(|i| hex_corner(center, hex_size, i)).collect();
        for v in vertices {
            min_x = min_x.min(v.x);
            max_x = max_x.max(v.x);
            min_y = min_y.min(v.y);
    ...
    // Créer un rectangle englobant le bâtiment
    let vertices = vec![
        [min_x, min_y, 0.1],
        [max_x, min_y, 0.1],
        [max_x, max_y, 0.1],
    ...
    let mesh_handle = meshes.add(mesh);
    let material_handle = materials.add(ColorMaterial::from(Color::rgb(0.8, 0.3, 0.3))); // Couleur rouge pour les bâtiments
